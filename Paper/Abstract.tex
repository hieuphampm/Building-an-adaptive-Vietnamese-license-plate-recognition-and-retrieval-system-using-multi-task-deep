...
\begin{document}
	...
	\begin{figure}[H]
		...
		\caption{Qualitative results of Module 1. The fine-tuned YOLOv8-nano model successfully localizes license plates under various challenging conditions, including different viewing angles, distances, and vehicle types.}
	...
	\begin{figure}[H]
		...
		\caption{The 3-branch conditional routing logic of the QAM, illustrated with representative real-world samples for each category. Based on the classification, an image is either sent for recognition, restoration, or removed.}
	...
	As analyzed in Section 2.3 (Related Work), GAN and Diffusion models, although powerful, carry the risk of fabricating details—an unacceptable risk for OCR tasks. Therefore, we prioritize an efficient Transformer-based model that maintains structural fidelity.
	
	We chose \textbf{Swin2SR} \cite{conde2022swin2sr} as the backbone architecture for this module. Swin2SR combines the hierarchical Swin Transformer V2 architecture with compressed super-resolution objectives, making it ideal for our constraints:
	\begin{enumerate}
		\item \textbf{Efficiency}: It is designed for efficient processing, suitable for the latency requirements of a multi-stage ALPR pipeline.
		\item \textbf{Text Preservation}: Swin2SR excels at reconstructing high-frequency details from low-resolution, blurry, or noisy inputs—exactly the types of errors our QAM is designed to detect.
	\end{enumerate}
	
	\begin{figure}[H]
		\centering
		\includegraphics[width=\linewidth]{Images/img_m3_architec.png}
		\caption{The architecture of the proposed Swin2SR \cite{conde2022swin2sr}.}
